...
class ApiHealthCheckMiddleware:
    ...
    def _detect_framework(self) -> str:
        ...
        """
        # Check for FastAPI instance or wrapped app (FastAPI has an 'app' attribute)
        if isinstance(self.app_core, FastAPI):
            return "fastapi"

        # Check if it's Flask
        elif isinstance(self.app_core, Flask):
            return "flask"

        # If it's a middleware wrapped app, check the underlying app
        elif isinstance(self.app_core, BaseHTTPMiddleware):
            if isinstance(self.app_core.app, FastAPI):
                return "fastapi"
            elif isinstance(self.app_core.app, Flask):
                return "flask"

        else:
            raise Exception(f"Unsupported framework: {type(self.app_core)}")
